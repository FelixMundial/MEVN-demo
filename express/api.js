...
router.post('/api/upload/avatar', (req, res, next) => {
  ...
  form.encoding = 'utf-8'
  form.uploadDir = './static/images' + AVATAR_DIR
  form.keepExtensions = true
  form.maxFieldsSize = 5 * 1024 * 1024
  form.parse(req, (err, fields, files) => {
    var parsedFile = files.file
    var parsedIndex = fields.index
    console.log('parsedFiles: ' + parsedFile)
    console.log('parsedIndex: ' + parsedIndex)
    if (err || parsedFile === undefined) {
      res.json({
        status: 500,
        msg: 'Parsing failure when uploading',
        result: ''
      })
    } else if (parsedFile.size > form.maxFieldsSize) {
      ...
      //   })
      // }
      var uploadTime = datetime.format(new Date(), 'YYYYMMDD')
...
router.get('/api/upload/image/loadAll', (req, res, next) => {
  console.log('/api/upload/image/loadAll')
  var SLIDER_DIR = '/slider'
  ...
  res.send(iteratedFiles)
})

router.get('/api/upload/avatar/loadAll', (req, res, next) => {
  console.log('/api/upload/avatar/loadAll')
  var AVATAR_DIR = '/avatar'
  var avatarDir = './static/images' + AVATAR_DIR
  var iteratedFiles = []
  function IterateFiles (dir) {
    var files = fs.readdirSync(dir)
    if (files && files.length) {
      files.forEach((filename) => {
        ...
        if (stats) {
          ...
          var isDir = stats.isDirectory()
          if (isFile) {
            iteratedFiles.push({avatarUrl: fileFullName})
          }
          if (isDir) {
            IterateFiles(fileFullName)
          }
